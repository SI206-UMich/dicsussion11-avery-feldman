...
def create_patients_table(cur, conn):
    ...
    cur.execute("CREATE TABLE Patients (pet_id INTEGER PRIMARY KEY, species_id INTEGER, name TEXT, age INTEGER, cuteness INTEGER, aggressiveness INTEGER)")
    conn.commit()
    pass
...
def non_aggressive_pets(aggressiveness, cur, conn):
    ...
    non_aggressive_lst = cur.fetchall()
    non_aggressive_lst = [item[0] for item in non_aggressive_lst]
    return non_aggressive_lst
    pass
